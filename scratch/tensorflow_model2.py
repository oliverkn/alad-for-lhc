...
class LinModel:
    ...
    def fit(self, x, y, n, batch_size, logdir):
        ...
        for epoch in range(n):
            ...
            loss_metric = tf.Variable(0., dtype=tf.float32, trainable=False)
            self.sess.run(tf.assign(loss_metric, 0))
            loss_op = tf.assign(loss_metric, loss_metric + self.loss)
            loss = 0
            ...
            for b in range(n_batch - 1):
                l = b * batch_size
                r = l + batch_size
                _, loss_value = self.sess.run((self.train_op, loss_op),
                                              feed_dict={self.x_in: x_feed[l:r], self.y_true: y_feed[l:r]})
                loss += loss_value

            loss_metric_val = self.sess.run(loss_metric)
            print("loss=%s, c=%s" % (loss_metric_val, sess.run(self.c_var)))
